...
{
    public struct ScriptGeneratingKit
    {
        static void CreateScript(string className, Action<StreamWriter> write)
        {
            string path = string.Format("{0}/{1}.cs", AssetKit.activeDirectory, className);
...
@"    }
        ...
        static string GetVariableName(string originalName, string fallbackName, List<string> list)
        {
            ...
            for (int i = 0; i < chars.Count; i++)
            {
                ...
                if (i == 0) invalid = (c != '_' && !TextKit.IsEnglishLetter(c));
                else invalid = (c != '_' && !TextKit.IsDigit(c) && !TextKit.IsEnglishLetter(c));
